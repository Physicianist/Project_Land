...
const navItems = [
  ...
  { id: 'pain', label: 'Когда сервис нужен' },
  { id: 'workflow', label: 'Как это работает' },
...
const heroHighlights = [
  'Подходят фото тетради, сканы и PDF',
  'AI собирает черновик проверки, но итог подтверждает преподаватель',
  'Комментарии, аналитика и отчеты формируются в одном кабинете',
];

const proofItems = [
  'Распознает почерк и переведет решение в удобный печатный вид',
  'Соберет черновик проверки с ошибками, баллом и комментарием',
  'Подскажет, где ошибка ученика, а где нужна ручная перепроверка',
  'Поможет быстро отправить понятный комментарий по работе',
  'Подготовит отчет родителям за выбранный период',
];
...
const painItems = [
  ...
  'Родители требуют обратной связи, и каждому нужно ответить',
];
...
const workSteps = [
  {
    ...
    text: 'Задайте тему, дедлайн и получателя. При необходимости добавьте ссылки и вложения, чтобы ученик сразу видел полный контекст.',
  },
  {
    number: '02',
  ...
  },
  {
    ...
    text: 'Сервис распознает текст, подсветит вероятные ошибки и подготовит черновик комментария с предложенным баллом. Вам останется проверить и утвердить итог.',
  },
  {
    number: '04',
    title: 'Аналитика результатов и отчёты',
    text: 'В кабинете доступны понятные дашборды по ученикам и группам. Система также собирает статистику и формирует отчеты для родителей по выбранному периоду.',
  },
];
...
const faqs = [
  ...
  },
  {
    ...
    a: 'После 30 дней Pro Trial преподаватель может остаться на Free с пакетной проверкой или перейти на Pro за 1490 ₽ в месяц',
  },
  {
  ...
  },
  {
    ...
    a: 'Черновик проверки всегда можно отредактировать вручную. Результат не отправляется ученику автоматически без подтверждения преподавателя',
  },
];
      ...
      <div className="plContainer plHeroGrid">
        <div className="plHeroCopy" data-reveal style={getRevealStyle(0)}>
          <h1 className="plHeroTitle">
            Проверка письменных работ без рутины, но с контролем преподавателя
          </h1>
          <p className="plHeroLead">
            ПроверьAI помогает быстрее разобрать фото тетрадей, сканы и PDF, собрать черновик
            комментария и не потерять качество обратной связи ученику и родителю.
          </p>
          <div className="plHeroChecklist">
            {heroHighlights.map(item => (
              <div key={item} className="plHeroChecklistItem">
                <span className="plHeroChecklistDot" aria-hidden="true" />
                <span>{item}</span>
              </div>
            ))}
          </div>
    ...
    <section className="plProofStrip">
      <div className="plContainer">
        <h3 className="plProofTitle" data-reveal style={getRevealStyle(0)}>
          Что получает преподаватель в реальном рабочем процессе
        </h3>
      </div>
    ...
    <section id="pain" className="plSection plSectionDark">
      <div className="plContainer plPainGrid">
        <div data-reveal style={getRevealStyle(0)}>
          ...
          <h2 className="plSectionTitleLight">Если узнаете себя хотя бы в одном пункте, сервис уже может сэкономить вам время</h2>
        </div>
    ...
    <section id="workflow" className="plSection plSectionTint">
      <div className="plContainer">
        <div className="plSectionHead" data-reveal style={getRevealStyle(0)}>
          ...
          <h2>Понятный сценарий от задания до подтвержденного результата</h2>
          <p>
            Сервис не заставляет менять привычный способ работы. Он просто убирает самую
            трудоемкую часть проверки и оставляет итоговое решение за преподавателем.
          </p>
        </div>
    ...
    <section className="plSection plFinalSection">
      <div className="plContainer">
        <div className="plFinalCard" data-reveal style={getRevealStyle(0)}>
          <p>
            Зарегистрируйтесь, загрузите первую работу и посмотрите, как проверка становится
            быстрее, а итоговый контроль по-прежнему остается у вас.
          </p>
